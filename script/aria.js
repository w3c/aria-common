...
/* global updateReferences, respecUI */
            ...
            $.each(document.querySelectorAll("rdef"), function (i, item) {
                                ...
                                function (i, item) {
                                    ...
                                    var type =
                                        item.localName === "pref"
                                            ? "property"
                                            : "state";
                                    var req = $(node).hasClass(
                                        "role-required-properties"
                                    );
                                    var dis = $(node).hasClass(
                                        "role-disallowed"
                                    );
                                    var dep = item.hasAttribute(
                                        "data-deprecated"
                                    );
                                    attrs.push({
                                        is: type,
                ...
                if (!isAbstract) {
                    $.each(
                        ...
                        function (i, node) {
                            var reqRef = container.querySelector(
                                ".role-namerequired"
                            );
                            var req = "";
                            ...
                            if (
                                node.textContent.indexOf("encapsulation") !== -1
                            ) {
                                fromEncapsulation +=
                                    '<li><a href="#' +
                                    pnID +
            ...
            if (!skipIndex) {
                ...
                $.each(propList, function (i, item) {
                    ...
                    if (
                        ...
                        // update the used in roles list
                        let sortedList = item.roles.sort();
                ...
                for (let i = 0; i < subRoles.length; i++) {
                    let item = subRoles[subRoles[i]];
                    let sortedList = item.sort(function (a, b) {
