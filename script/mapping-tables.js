        ...
        $("summary", mappingTableInfo.detailsContainer).each(function () {
            $(this).attr(
                ...
                mappingTableInfo.ids[
                    $("details", mappingTableInfo.detailsContainer).index(
                        $(this).parent()
                    )
                ]
            );
    ...
    function mappingTables() {
        $(".table-container").each(function () {
            ...
            tableInfo.detailsContainer = $(
                '<div class="details removeOnSave" id="' +
                    tableInfo.table.attr("id") +
                    '-details"></div>'
            );
            ...
            var $viewSwitch = $(
                '<button class="switch-view removeOnSave">' +
                    mappingTableLabels.viewByTable +
                    "</button>"
            ).on("click", function () {
                ...
                if (tableInfo.detailsContainer.is(":visible")) {
                    ...
                    $(this).text(
                        mappingTableLabels.viewByLabels[
                            tableInfo.table.attr("id")
                        ]
                    );
                } else {
            ...
            $("tbody tr", tableInfo.table).each(function () {
                ...
                if (tableInfo.table.hasClass("attributes")) {
                    ...
                    var relevantElsCaption = rowCells.shift(),
                        relevantElsSummary = relevantElsCaption.replace(
                            /<a [^>]+>|<\/a>/g,
                            ""
                        );
                }
                ...
                var details =
                    '<details class="map removeOnSave"><summary id="' +
                    id +
                    '">' +
                    $summary;
            ...
            var $expandAllButton = $(
                '<button class="expand removeOnSave">' +
                    mappingTableLabels.expand +
                    "</button>"
            );
            var $collapseAllButton = $(
                '<button disabled="disabled" class="collapse removeOnSave">' +
                    mappingTableLabels.collapse +
                    "</button>"
            );
            tableInfo.detailsContainer.prepend(
                $expandAllButton,
                $collapseAllButton
            );
            ...
            var $showHideCols = $(
                '<div class="show-hide-cols removeOnSave"><span>' +
            ...
            for (var i = 0, len = colHeaders.length; i < len; i++) {
                ...
                var $showHideColButton = $(
                    '<button class="hide-col" aria-pressed="false" title="' +
                    ...
                    if ($(this).attr("class") == "hide-col") {
                        ...
                        $(this).attr({
                            class: "show-col",
                            "aria-pressed": "true",
                            title: mappingTableLabels.showToolTipText,
                        });
                        $("span", $(this)).text(
                        ...
                        $(this).attr({
                            class: "hide-col",
                            "aria-pressed": "false",
                            title: mappingTableLabels.hideToolTipText,
                        });
                        $("span", $(this)).text(
                            mappingTableLabels.hideActionText
                        );
        ...
        var setExpandCollapseButtons = function ($detCont) {
            ...
            var detailsOpen = $detCont.find("details.open, details[open]")
                .length;
            //if, after the details is opened or closed...
            if (detailsOpen == totalDetails) {
                //all details are open, enable collapse all button and disable expand all button
                $detCont.find("button.collapse").removeAttr("disabled");
                $detCont.find("button.expand").attr("disabled", "disabled");
            } else if (totalDetails > detailsOpen && detailsOpen > 0) {
                //some but not all details are open, enable collapse all button
                $detCont.find("button.collapse").removeAttr("disabled");
                $detCont.find("button.expand").removeAttr("disabled");
            } else {
                //no details are open, disable collapse all button and enable expand all button
